...
6. **Themed minimal UI.** Solarized Light default; ten themes switchable in Settings (`Theme.swift` — one struct literal per theme: Solarized Light/Dark, Gruvbox Light/Dark, Catppuccin Latte/Mocha, Nord, Dracula, Tokyo Night, Mono Dark). System font (SF Pro — Charles found a pixel font "too much"), **no border** (he disliked it), cornerRadius 16 (Raycast-2.0-inspired, 2026-07 — was 6), instant show/hide, inset rounded selection (foreground @ 10%, radius 10, inset 14pt from the panel edge — replaced the inverted block 2026-07; Charles explicitly declined Raycast's glass/blur, row icons, and footer bar), per-kind **TheyDo-style chips** on the right of each row (2026-07, replaced the old rainbow badges Charles disliked): one shared hue set `Theme.theydoKinds` (violet/pink/teal/amber/blue/ink-gray/orange for lists), rendered as a light pastel **gradient** fill + same-hue tinted text via `Theme.chip(for:)`, derived against each theme's background/foreground so chips adapt to light/dark — `kindColors` stays per-theme-overridable (Mono Dark overrides to stay monochrome). No `>` prompt glyph in the query row (removed 2026-07). Fixed-height 8-row **scrollable** list viewport (empty query lists *all* destinations, scroll or arrow through them; the panel never resizes while filtering — only on stage transitions), dim "ms" filter-latency readout. Placeholder text must use `prompt: Text(...).foregroundColor(theme.dim)` — default placeholders ignore the theme and vanish on dark themes.
...
Sources/
  ...
  DestinationListView.swift Fixed-height scrollable viewport, TheyDo chips right-aligned, inset rounded selection
...
- **`hidePanel` is EXACTLY `orderOut(nil)` — nothing else** (changed 2026-07 after the hover bug recurred). No focus hand-back, no `makeFirstResponder(nil)`, no `activate()`. The panel is `.nonactivatingPanel` and we only `orderFrontRegardless()` + `makeKey()`, so our `.accessory` app never becomes active — the previously-focused app stays active and macOS returns its key window natively on `orderOut`.
  - **Why the hand-back was removed:** `showPanel` used to snapshot `frontmostApplication` and `hidePanel` used to call `previous.activate(from: .current, options: [])` (guarded). It ran on every hide and was the *only* thing actively reaching into another process's window state — the leading suspect for the slow, multi-day system-wide hover/tooltip decay in *other* apps (Slack, Dia). It was insurance against an unproven "orderOut return decays over days" theory; v0.2.0's plain `orderOut` was clean per-cycle. When the bug recurred with the hand-back in place, it was ripped out. This is a multi-day soak bug — per-cycle keyboard tests give false negatives (key return can work while hover return is broken).
  - Still FORBIDDEN if ever re-adding hide-path logic: **`makeFirstResponder(nil)` before `orderOut`** (kills AppKit key-window return EVERY cycle — the v0.3.1 regression, 100% reproducible) and **`becomesKeyOnlyIfNeeded = true`** (makes key return flaky, 1 in 3). Panel keeps the NSPanel default (`false`); key is taken via explicit `makeKey()` in `showPanel`.
...
- **No "panel resigned key" as a dismiss signal.** Non-activating panels routinely lose key status for OS-internal reasons; `didResignKeyNotification`-based dismissal made the panel vanish ~1s after showing. Dismiss only on Esc / Enter / hotkey toggle — all of which funnel through `hidePanel()`; keep that invariant.
