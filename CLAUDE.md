...
- **Panel settings** (`Sources/LauncherPanel.swift`):
  - `hidesOnDeactivate = true` (2026-08). Redundant on purpose with the resign-key observer, matching Gyors. It was `false` until 2026-08 because, while the app never activated, the system fired app-deactivated almost immediately and this auto-hid the panel on every show after the first. Once `showPanel` activates, no spurious deactivation arrives. If that regression ever returns, flip this back to `false` and rely on the observer alone.
