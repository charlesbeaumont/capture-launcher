...
- **No "panel resigned key" as a dismiss signal.** Non-activating panels routinely lose key status for OS-internal reasons; `didResignKeyNotification`-based dismissal made the panel vanish ~1s after showing. Dismiss only on Esc / Enter / hotkey toggle.
