...
- **`hidePanel` is exactly `orderOut` + hand-back — two calls are FORBIDDEN in the hide path** (both verified 2026-07-15 with a mouseMoved-counting victim window; both make hover/tooltips die in the *focused app behind the panel*, the "system-wide hover bug"):
  - **`makeFirstResponder(nil)` before `orderOut`** kills AppKit's key-window return to the previous app on EVERY cycle (this was the v0.3.1 regression — 100% reproducible).
  - **`becomesKeyOnlyIfNeeded = true`** makes key-window return FLAKY (failed 1 in 3 cycles). The panel keeps the NSPanel default (`false`); key is taken via the explicit `makeKey()` in `showPanel`.
